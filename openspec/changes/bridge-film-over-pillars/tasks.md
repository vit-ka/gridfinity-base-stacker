...
- [ ] 1.1 Add a connected-component count over a bitmask raster (flood fill on
...
- [ ] 1.2 Report disconnected film regions per gap in the stack report, and
      ...
      recorded 22.6 cm3
- [ ] 1.3 Record, for each island, the shortest span from it to any other base
...
- [ ] 2.1 Add a closing step to `interface_slabs`: `dilate` then `erode` by
...
- [ ] 3.2 Set the default to 6 mm and record what it costs in the option's help
...
- [ ] 3.3 Quantify the closing's side effect: how much of the added volume joins
...
- [ ] 4.1 Generate the stack and film at the chosen default, build the 3mf with
...
- [ ] 4.2 Parse the G-code for the bridge spans specifically: verify they print
      as bridge or solid infill and not as air, and record which line types
      appear over the pillar tops
- [ ] 4.3 Verify no slicer support appeared anywhere on the model, the check the
...
- [ ] 5.1 Add the film region count to `verify.py`, derived from the written STL
      rather than from the plan, and verify it reports the same per-gap counts
      as the generator did in 3.1
- [ ] 5.2 Run the existing suite and `verify.py` end to end on the generated
      files, and verify the film still clears both plates by the configured
      clearance
- [ ] 5.3 Write the trim ordering into `trim-unsupporting-film`'s tasks -- trim
